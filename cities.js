...
function averagePopulation(citiesData, island) {
  const onlyIsland = _.filter(citiesData, (x) => x.island === island);
  const totalPopulation = _.reduce(onlyIsland, (acc, x) => x.population + acc, 0);
  return totalPopulation / onlyIsland.length;
}

function biggestAndSmallest(citiesData, island) {
  const onlyIsland = _.filter(citiesData, (x) => x.island === island);
  const biggest = _.max(onlyIsland, 'population');
  const smallest = _.min(onlyIsland, 'population');
  return `Biggest: ${biggest.name}; Smallest: ${smallest.name}`;
}

console.log(averagePopulation(cities, 'Oahu')); // prints: 20346.85714285714
console.log(biggestAndSmallest(cities, 'Oahu')); // prints: Biggest: Urban Honolulu; Smallest: Maunawili
console.log(averagePopulation(cities, 'Maui')); // prints: 9390.4375
